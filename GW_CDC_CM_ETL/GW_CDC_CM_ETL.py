...
primary_key = "id"  
# for stage 
secret_name = "PreprodPersistent_CM"  
GWreadReplica_Creds = "GWreadReplica_Creds_CM_Stage"  
# for prod
# secret_name = "PostgresPersistent_CM"
# GWreadReplica_Creds = "GWreadReplica_CM"
...
def process_table(table_name, run_id, prev_run_start_time, curr_run_start_time, details_count):
    ...
    # ---------------------------------------------
    fetch_scenario_query = f"""SELECT *	FROM public.gw_cdc_metadata_config where metatablename = '{table_name}' and metaschema = 'contactmanager' """
    for attempt in range(1, max_retries + 1):
        try:
    ...
    for row in scenario_info_df.collect():
        try:
            if (str(row["metacategory"]) == "1") or (str(row["metacategory"]) == "4"): 
                ...
                details_update_query = f"update gw_cdc_meta_runtimes_cm_detail set runendtime = '{datetime.now(ZoneInfo('America/Chicago'))}', RunJobId = '{job_id}',  runtableloadcompletedflag = 'Y' where runid = {run_id} and runtablename = '{table_name}'"
                ...
                try:
                    ...
                    update_table_from_query(details_update_query, persistent_post_db_options, table = 'gw_cdc_meta_runtimes_cm_detail')
                    update_table_from_query(truncate_query, persistent_staging_db_options, table_name)
                    
                    update_table_from_query(drop_query, insert_db_options, table = f"updates_temp_{table_name}")
                except Exception as e:
                    log_db_name = persistent_post_db_options['url'].split('/')[3]
                    logger.error(f"Error while inserting record into gw_cdc_meta_runtimes_cm_detail for table {table_name} of {log_db_name} database : {str(e)}")
                    return
                ...
                if details_count is None:
                    ...
                    try:
                        ...
                        log_db_name = persistent_post_db_options['url'].split('/')[3]
                        logger.error(f"Error while inserting count record into gw_cdc_meta_runtimes_cm_detail for table {table_name} of {log_db_name} database : {str(e)}")
                        return
                ...
                try:
                    ...
                    logger.error(f"Error while inserting records into gw_cdc_meta_runtimes_cm_detail for table {table_name} of {log_db_name} database : {str(e)}")
                    return
                logger.info(f"The {table_name} table process has completed successfully.")
...
def delete_records_from_rds_for_update(table_name, unmatched_df, db_options, action):
    ...
    delete_sql = f"""
            DELETE FROM public.{table_name}
            USING cm_staging_fdw.{table_name}
            WHERE public.{table_name}.{primary_key} =  cm_staging_fdw.{table_name}.{primary_key}
        """
    logger.info(f"delete_sql : {delete_sql}")
    conn = psycopg2.connect(
            host=db_options["url"].split("://")[1].split(":")[0],  # Extract host from URL
...
def fetch_latest_metarun_info(meta_runtime_query):
    ...
    if idstarttime_lst_df.rdd.isEmpty():
        ...
        try:
            meta_df.write.jdbc(url=secrets["persistent"]["url_p"], table='gw_cdc_meta_runtimes_cm', mode="append", properties = persistent_post_db_options)
            logger.info(f"meta_runtimes_detail data initialized successfully into gw_cdc_meta_runtimes_cm ")
        except Exception as e:
            logger.error(f"Error while initializing records into the gw_cdc_meta_runtimes_cm  : {e}")
            raise
        restart_flag = 'N'
        ...
        if run_completed_flag == 'Y':
            ...
            try:
                meta_df.write.jdbc(url=secrets["persistent"]["url_p"], table='gw_cdc_meta_runtimes_cm', mode="append", properties = persistent_post_db_options)
                logger.info(f"meta_runtimes_detail data initialized successfully for gw_cdc_meta_runtimes_cm")
            except Exception as e:
                logger.error(f"Error while initializing records into the gw_cdc_meta_runtimes_cm for gw_cdc_meta_runtimes_cm : {e}")
                raise
            restart_flag = 'N'
...
with ThreadPoolExecutor(max_workers=19) as executor:
    ...
    meta_runtime_query = f"select runid, runstarttime, runcompletedflag from public.gw_cdc_meta_runtimes_cm"
    ...
    if restart_flag == "N":
        ...
        table_lst_query = f"""SELECT metatablename, null as runtableinsertcount FROM gw_cdc_metadata_config where metaschema = 'contactmanager'"""
        logger.info(f"table_lst_query : {table_lst_query}")
        
        # Fetch full metadata config details including hashpartition and type flags
        fetch_metadataconfig_query = f"""SELECT * FROM public.gw_cdc_metadata_config where metaschema = 'contactmanager'"""
        ...
        try:
            meta_df.write.jdbc(url=secrets["persistent"]["url_p"], table='gw_cdc_meta_runtimes_cm_detail', mode="append", properties = persistent_post_db_options)
            logger.info(f"gw_cdc_meta_runtimes_cm_detail data initialized successfully for {meta_df.count()} tables")
        except Exception as e:
            logger.error(f"Error while initializing record into the gw_cdc_meta_runtimes_cm_detail: {e}")
            raise
    else:
        # For restart, only pick tables that were not completed in the previous run
        table_lst_query = f"SELECT runtablename as metatablename, runtableinsertcount  FROM gw_cdc_meta_runtimes_cm_detail where runid = {run_id} and runtableloadcompletedflag = 'N'"
        logger.info(f"table_lst_query : {table_lst_query}")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            logger.error(f"Attempt {attempt}: Error while retriving list of table names from the gw_cdc_meta_runtimes_cm_detail table: {str(e)}")
            if attempt < max_retries:
                logger.error(f"Retrying in {retry_delay} seconds...")
                time.sleep(retry_delay)
            else:

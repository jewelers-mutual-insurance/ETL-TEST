...
def process_table(table_name, run_id, prev_run_start_time, curr_run_start_time, details_count):
    # If no existing run details are found, attempt to update the runstarttime in the metadata table.
    # Retries the update on failure, up to a maximum number of attempts with delay.
    if details_count is None:
        for attempt in range(1, max_retries + 1):
            try:
                update_runstarttime_query = f"update gw_cdc_meta_runtimes_pc_detail set runstarttime = '{datetime.now(ZoneInfo('America/Chicago'))}' where runid = {run_id} and runtablename = '{table_name}'"
                update_table_from_query(update_runstarttime_query, persistent_post_db_options, table = 'gw_cdc_meta_runtimes_pc_detail')
                break
            except Exception as e:
                logger.error(f"Attempt {attempt}:Failed to update 'runstarttime' in gw_cdc_meta_runtimes_pc_detail for table : {str(e)}")
                if attempt < max_retries:
                    logger.error(f"Retrying in {retry_delay} seconds...")
                    time.sleep(retry_delay)
                else:
...
def fetch_latest_metarun_info(meta_runtime_query):
    ...
    # Efficient logic using only one Spark action
...
with ThreadPoolExecutor(max_workers=14) as executor:
    ...
    if restart_flag == "N":
        table_lst_query = f"""SELECT metatablename, null as runtableinsertcount FROM gw_cdc_metadata_config where metaschema = 'policycenter' and meta_worker = 4"""
        logger.info(f"table_lst_query : {table_lst_query}")
    else:
        table_lst_query = f"SELECT runtablename as metatablename, runtableinsertcount FROM gw_cdc_meta_runtimes_pc_detail where runid = {run_id} and runtableloadcompletedflag = 'N' and runtablename in (SELECT metatablename FROM public.gw_cdc_metadata_config where metaschema = 'policycenter' and meta_worker = 4)"
        logger.info(f"table_lst_query : {table_lst_query}")
